...
export type AggregatePoint = { snapshotKey: string; capturedAt: string; value: number };
export type AccountPoint = { snapshotKey: string; capturedAt: string; accountKey: string; value: number };
export type AccountHistory = { aggregatePoints: AggregatePoint[]; accountPoints: AccountPoint[] };
...
export function AccountsNetWorthTracker({ rows, netWorth, totalAssets, totalLiabilities, historyPromise }: { rows: AccountBreakdownRow[]; netWorth: number; totalAssets: number; totalLiabilities: number; historyPromise: Promise<AccountHistory> }) {
  const [selectedAccountId, setSelectedAccountId] = useState<string | null>(null);

  return <div className="space-y-5">
    <Suspense fallback={<AccountHistoryFallback />}>
      <AccountHistoryChart historyPromise={historyPromise} rows={rows} selectedAccountId={selectedAccountId} setSelectedAccountId={setSelectedAccountId} />
    </Suspense>
...
function AccountHistoryChart({ historyPromise, rows, selectedAccountId, setSelectedAccountId }: { historyPromise: Promise<AccountHistory>; rows: AccountBreakdownRow[]; selectedAccountId: string | null; setSelectedAccountId: (value: string | null) => void }) {
  const { aggregatePoints, accountPoints } = use(historyPromise);
  ...
  return <section aria-labelledby="accounts-net-worth-history" className="overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
...
function AccountHistoryFallback() {
  return (
    <section aria-label="Loading net worth history" className="overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
      <div className="border-b border-zinc-100 px-5 py-4 dark:border-zinc-900">
        ...
        <div className="mt-2 h-3 w-64 max-w-full animate-pulse rounded bg-zinc-100 dark:bg-zinc-900" />
      </div>
      <div className="h-72 animate-pulse bg-zinc-50 dark:bg-zinc-900/40" />
    </section>
  );
}
