...
import { effectiveCashFlow, pairCreditCardPayments } from "@/lib/transaction-transfers";
...
export default async function TransactionsPage({ searchParams }: { searchParams: Promise<{ category?: string }> }) {
  ...
  await Promise.all([autoCategorizeTransactions(userId), pairCreditCardPayments(userId)]);
  const transactions = await prisma.plaidTransaction.findMany({
    ...
    include: { transactionCategory: true, transferPairAsDebit: { select: { id: true } }, transferPairAsCredit: { select: { id: true } }, splits: { include: { category: true } }, plaidAccount: { include: { plaidItem: { select: { institutionName: true } } } } },
  ...
  for (const transaction of transactions) {
    ...
    if (cashFlow === "INCOME") { if (bucket) bucket.income += Math.abs(transaction.amount); continue; }
    if (cashFlow !== "SPENDING") continue;
    const spendingAmount = Math.abs(transaction.amount);
    if (bucket) bucket.spending += spendingAmount;
    const contributions = transaction.splits.length ? transaction.splits.map((split) => [split.category.name, split.amount] as const) : [[transaction.transactionCategory?.name ?? pretty(transaction.plaidDetailedCategory || transaction.plaidPrimaryCategory || "Other"), spendingAmount] as const];
  ...
  const rows: TransactionRow[] = transactions.map((transaction) => ({ id: transaction.id, date: transaction.date.toISOString(), name: transaction.name, merchantName: transaction.merchantName, amount: transaction.amount, pending: transaction.pending, categoryId: transaction.transactionCategoryId, category: transaction.transactionCategory?.name ?? null, categorizationSource: transaction.categorizationSource, expenseTreatment: transaction.expenseTreatment, cashFlowTreatment: transaction.cashFlowTreatment, effectiveCashFlow: effectiveCashFlow(transaction), transferMatched: Boolean(transaction.transferPairAsDebit || transaction.transferPairAsCredit), accountName: transaction.plaidAccount.name, accountMask: transaction.plaidAccount.mask, institutionName: transaction.plaidAccount.plaidItem.institutionName, splits: transaction.splits.map((split) => ({ id: split.id, categoryId: split.categoryId, category: split.category.name, amount: split.amount, note: split.note })) }));
...
function buildInsights(transactions: Array<{ date: Date; amount: number; pending: boolean; merchantName: string | null; name: string; plaidPrimaryCategory: string | null; plaidDetailedCategory: string | null; cashFlowTreatment: "AUTO" | "SPENDING" | "INCOME" | "TRANSFER"; transactionCategory: { name: string; slug: string } | null; transferPairAsDebit: { id: string } | null; transferPairAsCredit: { id: string } | null }>, months: CashFlowMonth[], categoryByMonth: Map<string, Map<string, number>>): CashFlowInsights {
  ...
  for (const transaction of transactions) { if (transaction.pending || effectiveCashFlow(transaction) !== "SPENDING") continue; const label = transaction.merchantName || transaction.name; const key = normalizeMerchant(label); const group = recurringGroups.get(key) ?? []; group.push({ date: transaction.date, amount: Math.abs(transaction.amount), label }); recurringGroups.set(key, group); }
  ...
  const priorYearSpending = transactions.filter((transaction) => !transaction.pending && monthKey(transaction.date) === priorYearKey && effectiveCashFlow(transaction) === "SPENDING").reduce((sum, transaction) => sum + Math.abs(transaction.amount), 0);
